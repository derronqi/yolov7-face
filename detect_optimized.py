...
def detect(inputs, outputs, bindings, context, opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred = forward_pass(img, inputs, outputs, bindings, context)[0]
        pred = pred.reshape(1, -1, 21)
        print(pred.shape)
        print(pred[...,4].max())
        # Apply NMS
        pred = non_max_suppression(torch.from_numpy(pred), opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label)
...
def forward_pass(x, inputs, outputs, bindings, context):
        ...
        with engine.create_execution_context() as context:
            ...
            if opt.update:  # update all models (to fix SourceChangeWarning)
                for opt.weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
                    detect(inputs, outputs, bindings, context, opt=opt)
